...
const config = {
  ...
  computedBasedOn: 'computed-location-source-data',
  weatherSource: 'accuweather.0',
  lovelaceCompatibleWeatherDevice: 'alias.0.GW.Next Event Weather',
};
        ...
        acc[`Day_${humanizedDay}`] = {
          ...
          nested: {
            location: state({
              type: 'string',
              role: 'location',
              source: `${config.root}.${config.computed}`,
            }),
            date: state({
              type: 'string',
              role: `date.forecast.${day}`,
              source: `${config.weatherSource}.Summary.DateTime_d${humanizedDay}`,
            }),
            state: state({
              type: 'string',
              role: `weather.state.forecast.${day}`,
              source: `${config.weatherSource}.Summary.WeatherText_d${humanizedDay}`,
            }),
            icon: state({
              type: 'string',
              role: `weather.icon.forecast.${day}`,
              source: `${config.weatherSource}.Summary.WeatherIconURL_d${humanizedDay}`,
            ...
            temp_min: state({
              ...
              source: `${config.weatherSource}.Summary.TempMin_d${humanizedDay}`,
            }),
            temp_max: state({
              type: 'number',
              unit: '°C',
              role: `value.temperature.max.forecast.${day}`,
              source: `${config.weatherSource}.Summary.TempMax_d${humanizedDay}`,
            }),
            precipitation_chance: state({
              type: 'number',
              unit: '%',
              role: `value.precipitation.forecast.${day}`,
              source: `${config.weatherSource}.Summary.PrecipitationProbability_d${humanizedDay}`,
            }),
            precipitation: state({
              type: 'number',
              unit: 'mm',
              role: `value.precipitation.forecast.${day}`,
              source: `${config.weatherSource}.Summary.TotalLiquidVolume_d${humanizedDay}`,
            }),
            wind_direction_bearing: state({
              type: 'number',
              unit: '°',
              role: `value.direction.wind.forecast.${day}`,
              source: `${config.weatherSource}.Summary.WindDirection_d${humanizedDay}`,
            }),
            wind_direction: state({
              type: 'string',
              role: `weather.direction.wind.forecast.${day}`,
              source: `${config.weatherSource}.Summary.WindDirectionStr_d${humanizedDay}`,
            }),
            wind_speed: state({
              type: 'number',
              unit: 'km/h',
              role: `value.speed.wind.forecast.${day}`,
              source: `${config.weatherSource}.Summary.WindSpeed_d${humanizedDay}`,
            }),
          },
        };
...
async function updateComputedLocation(
  ...
  await setStateAsync(`${config.root}.${config.computed}`, locationName, true);
  await setStateAsync(
    `${config.root}.${config.computedBasedOn}`,
    basedOn,
    true,
  );
  ...
  await extendObjectAsync(config.lovelaceCompatibleWeatherDevice, {
...
const locationChanges = combineLatest([
  new Stream<string>(`${config.root}.${config.source}`).stream,
  new Stream<string>(`${config.root}.${config.computedBasedOn}`).stream,
]).pipe(
  ...
  tap(async loc => {
    ...
    const adapterConfigId = `system.adapter.${config.weatherSource}`;
    ...
    await setStateAsync(`${config.weatherSource}.updateDaily`, true, false);

    await updateComputedLocation(locationName, loc.next);
  }),
);

await ObjectCreator.create(nextEventExtension(), config.root);
